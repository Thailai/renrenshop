...
<link href="./resource/css/jquery.searchableSelect.css" rel="stylesheet" type="text/css">
<script src="./resource/js/lib/jquery.searchableSelect.js"></script>
<style>
...
<script type="text/javascript">
	$(function(){
		$(".mybtn").click(function(){
      	...
      	$(".myok").click(function(){
			var move_money = $("#move_money").val();
			var openid = $("#openid option:selected").val();
			var url = "<?php  echo mobileUrl('member/move')?>";
			$.post(url, { move_money: move_money, openid: openid },function(data){
          		if (data == "转移成功！") {
          			alert(data);
          			location.reload();
          		}else{
      	...
      	$("#openid").change(function(){
			var src = $("#openid option:selected").data("src");
			$("#member_img").attr("src",src);
		});
      	$('#openid').searchableSelect();
	})
</script>
<div class='fui-page  fui-page-current'>
	...
	<div class="mybox">
      	<div style="margin-top: 5%;margin-left: 5%;height: 15%;">转移余额：<input id="move_money" type='number' style="border: 1px solid;padding: 5px;" value=""></div>
      	<div style="margin-left: 5%;height: 15%;">
          	<div>转移到：<img style="height: 50px;width: 50px;" id="member_img" src=""></div>
			<select  id="openid">
				<option value="" data-src="" >----请选择会员----</option>
				<?php  if(is_array($memberAll)) { foreach($memberAll as $item) { ?>
				<option data-src="<?php  echo $item['avatar'];?>" value="<?php  echo $item['openid'];?>"><?php  echo $item['nickname'];?></option>
				<?php  } } ?>
			</select>
		</div>      	
		<div style="margin-top: 25%;margin-left: 25%;">
			<button class="btn btn-success myok">确定</button>
			<button class="btn btn-danger mycancel">取消</button>
		</div>
	</div>
			...
			<div class="headinfo" style="z-index:100;border: none;">
				...
				<div class="child">
					...
					<?php  if(empty($_W['shopset']['trade']['closerecharge'])) { ?><a href="<?php  echo mobileUrl('member/recharge')?>"><div class="btn">充值</div></a><?php  } ?>
                  	<a href="<?php  echo mobileUrl('member/withdraw')?>"><div class="btn">余额提现</div></a>
                  	&nbsp;<div class="btn mybtn">余额互转</div>
				</div>
